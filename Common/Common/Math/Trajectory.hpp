...
class Trajectory {
 public:
   ...
   */
  Trajectory(std::vector<Vec3d> coeffs, double startTime, double endTime)
      :
      _coeffs(coeffs),
   ...
   * @param endTime The latest time at which the polynomial is defined
   */
  Trajectory(Vec3d c0, Vec3d c1, Vec3d c2, Vec3d c3, Vec3d c4, Vec3d c5,
   ...
   */
  std::vector<Vec3d> GetDerivativeCoeffs() const {
    std::vector<Vec3d> derivCoeffs;
  ...
  inline Vec3d operator[](int i) const {
    switch (i) {
      ...
      default:
        ...
        assert(false);
        return Vec3d();  // Returns vector of NaNs
    }
  }

 private:
  std::vector<Vec3d> _coeffs;
