...
#include "Components/DepthImagePlanner/Pyramid.hpp"
#include "Components/DepthImagePlanner/MonotonicTrajectory.hpp"
...
inline TrajectoryTestResult operator~(TrajectoryTestResult a) {
  return (TrajectoryTestResult) ~(int) a;
}
inline TrajectoryTestResult operator|(TrajectoryTestResult a,
                                      TrajectoryTestResult b) {
  return (TrajectoryTestResult) ((int) a | (int) b);
}
inline TrajectoryTestResult operator&(TrajectoryTestResult a,
                                      TrajectoryTestResult b) {
  return (TrajectoryTestResult) ((int) a & (int) b);
...
class DepthImagePlanner {
 public:
  ...
  bool FindFastestTrajRandomCandidates(
      ...
      double allocatedComputationTime, Vec3d explorationDirection);
  ...
  void DeprojectPixelToPoint(double x, double y, double depth,
                             Vec3d &outPoint) {
    outPoint = depth
        * Vec3d((x - _cx) / _focalLength, (y - _cy) / _focalLength, 1);
  }
   ...
   */
  void ProjectPointToPixel(Vec3d point, double &outX, double &outY) {
  ...
  class RandomTrajectoryGenerator {
   public:
    ...
    RandomTrajectoryGenerator(DepthImagePlanner *planner)
        :
        //Enable efficient sampling
        _pixelX(0.1 * planner->GetImageWidth(), 0.9 * planner->GetImageWidth()),
        _pixelY(0.1 * planner->GetImageHeight(),
                0.9 * planner->GetImageHeight()),

//        //Disable efficient sampling
//        _pixelX(0.0 * planner->GetImageWidth(), 1.0 * planner->GetImageWidth()),
//        _pixelY(0.0 * planner->GetImageHeight(),
//                1.0 * planner->GetImageHeight()),
...
//        _time(2, 4),
    ...
    RandomTrajectoryGenerator(int minXpix, int maxXpix, int minYpix,
                              ...
                              double minTime, double maxTime, int rdSeed,
    ...
    int GetNextCandidateTrajectory(
      ...
      nextTraj.SetGoalVelocity(Vec3d(0, 0, 0));
      nextTraj.SetGoalAcceleration(Vec3d(0, 0, 0));
 ...
 private:
  ...
  class ExplorationCost {
   public:
    ...
    ExplorationCost(Vec3d explorationDirection)
        :
