...
DepthImagePlanner::DepthImagePlanner(cv::Mat depthImage, double depthScale,
    ...
    _minimumAllowedThrust(5),  // By default don't allow negative thrust
    _maximumAllowedThrust(30),  // By default limit maximum thrust to about 3g (30 m/s^2)
//    _maximumAllowedThrust(15),  // By default limit maximum thrust to about 3g (30 m/s^2)
    _maximumAllowedAngularVelocity(20),  // By default limit maximum angular velocity to 20 rad/s
//    _maximumAllowedAngularVelocity(10),  // By default limit maximum angular velocity to 20 rad/s
    _maximumAllowedVelocity(5),  // By default limit maximum velocity to 1 m/s
...
bool DepthImagePlanner::FindFastestTrajRandomCandidates(
    ...
    std::vector<TrajectoryTest> &trajectories, double allocatedComputationTime,
    Vec3d explorationDirection) {

  ExplorationCost explorationCost(explorationDirection);
  return FindLowestCostTrajectoryRandomCandidates(
      trajectory, trajectories, allocatedComputationTime, &explorationCost,
...
bool DepthImagePlanner::FindLowestCostTrajectoryRandomCandidates(
  ...
  return FindLowestCostTrajectory(
      trajectory, trajectories, allocatedComputationTime,
      costFunctionDefinitionObject, costFunctionWrapper, (void*) &trajGenObj,
...
bool DepthImagePlanner::FindLowestCostTrajectory(
  ...
  Vec3d pos0 = trajectory.GetPosition(0);
  Vec3d vel0 = trajectory.GetVelocity(0);
  Vec3d acc0 = trajectory.GetAcceleration(0);
  Vec3d grav = trajectory.GetGravityVector();
  ...
  while (true) {
    ...
    const bool KEEP_TRAJECTORIES = true;
    if (SKIP_COST_CHECK || cost < bestCost) {
      ...
      if (SKIP_FEASIBILITY_CHECK
          || res == RapidTrajectoryGenerator::InputFeasible) {
        ...
        RapidTrajectoryGenerator::StateFeasibilityResult isVelocityAdmissible =
            candidateTraj.CheckVelocityFeasibility(_maximumAllowedVelocity);
        if (SKIP_VELOCITY_CHECK
            || isVelocityAdmissible
                == RapidTrajectoryGenerator::StateFeasibilityResult::StateFeasible) {
          // The trajectory is velocity state feasible
          if (isVelocityAdmissible
              == RapidTrajectoryGenerator::StateFeasibilityResult::StateFeasible) {
    ...
    // if (_numTrajectoriesGenerated == 1000) {
    //   break;
    // }
    // assert (_numTrajectoriesGenerated <= 1000);
  }
...
std::vector<MonotonicTrajectory> DepthImagePlanner::GetMonotonicSections(
  ...
  std::vector<Vec3d> trajDerivativeCoeffs = trajectory.GetDerivativeCoeffs();
...
bool DepthImagePlanner::FindDeepestCollisionTime(MonotonicTrajectory monoTraj,
  ...
  std::vector<Vec3d> coeffs = monoTraj.GetCoeffs();
  for (Vec3d normal : pyramid.planeNormals) {
...
bool DepthImagePlanner::IsCollisionFreeGroundTruth(Trajectory trajectory) {
  ...
  for (double t = trajectory.GetStartTime(); t < trajectory.GetEndTime(); t +=
      timestep) {
    Vec3d trajPos = trajectory.GetValue(t);
    ...
    for (int y = 0; y < _imageHeight; y++) {
      for (int x = 0; x < _imageWidth; x++) {
        ...
        if (_depthData[y * _imageWidth + x] > ignoreDist) {
          ...
          Vec3d e = Vec3d((x - _cx) / _focalLength, (y - _cy) / _focalLength,
                          1.0).GetUnitVector();
