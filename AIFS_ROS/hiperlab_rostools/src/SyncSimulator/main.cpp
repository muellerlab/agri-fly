...
////////////////////////////////////////////////////////////////
//In SyncSimulator we sync each step with a ROS Sim time, so we
//can make sure the synthesized sensor feed is always in the time
//step we want.
////////////////////////////////////////////////////////////////
...
class SimVehicle {
 public:
  ...
  std::shared_ptr<ros::Subscriber> subDepthImageReceived;
  ...
  void callbackImage(const std_msgs::Header &msg) {
    if (not imageStarted) {
      imageStarted = true;
    }
    imageLock = false;
...
//  client.simSetVehiclePose(airsim_pose, true);
  ...
  v->id = vehicleId;
  v->imageStarted = false;
  v->subRadioCmd.reset(
      new ros::Subscriber(
          n.subscribe("radio_command" + std::to_string(vehicleId), 1,
                      &SimVehicle::callbackRadioCmd, v.get())));

  v->subDepthImageReceived.reset(
      new ros::Subscriber(
          n.subscribe("imageReceivedFlag", 1, &SimVehicle::callbackImage,
                      v.get())));
...
//    new image_transport::Publisher(it.advertise("depthImage", 1)));
  ...
  const double frequencyMocapOutput = 200;  //[Hz] Ideally, 1/mocapOuptputFreq is a constant multiple of 1/frequencySimulation.
...
//        Vec3d odometryPos = v->vehicle->GetPosition() - v->_initPos;
        ...
        // TODO !! Revise this and add skew symmetric term if necessary.
